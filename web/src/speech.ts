 ...
 *  always resolves even if the browser never fires `onend`. `rate` is the
 *  caller's per-role speed multiplier (0.7–1.4 in Settings); a small built-in
 *  per-voice bias (controller slightly faster/flatter) is layered on top. */
export function speak(text: string, voice: Voice = "controller", volume = 1, rate = 1): Promise<void> {
  ...
  return new Promise((resolve) => {
    try {
      ...
      u.rate = Math.min(2, (voice === "controller" ? 1.02 : 0.98) * rate);
...
export function playClip(blob: Blob, rate = 1, volume = 1): Promise<void> {
