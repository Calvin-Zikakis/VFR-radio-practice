...
public enum DrillLibrary {
    ...
    public static let towered: [Drill] = [
        ...
        Drill(
            ...
            setup: "You're at Monterey ready to taxi. Ground says: RV seven three seven juliet alpha, runway two eight right, taxi via Alpha, Charlie, cross runway one zero left, hold short of runway two eight left. Read back the full route.",
        ...
        Drill(
            ...
            scenario: .towered,
            title: "Parallel taxi — don't drop the runway",
            setup: "You've landed at Concord and exited runway three two left at hotel, taxiing back for another departure on three two left. Ground says: RV seven three seven juliet alpha, runway three two left, taxi via golf, papa, cross runway one niner left. Read it back.",
            situation: "Towered field, you are Concord Ground (Buchanan Field: parallel 32L/32R crossed by 19L/19R). You issued: 'runway three two left, taxi via golf, papa, cross runway one niner left'. Grade the readback: it MUST include the assigned runway ('runway three two left') — pilots concentrating on the route famously drop the runway itself; if it's missing, reply 'readback incomplete, say assigned runway' and do not advance. Also required: the crossing clearance ('cross runway one niner left') and the callsign; the route (golf, papa) belongs in there too. Parallel trap: 'three two right' instead of 'three two left' gets corrected immediately.",
        ...
        Drill(
            ...
            setup: "You landed on runway three two right at Concord and you're taxiing to depart on the parallel. Ground says: RV seven three seven juliet alpha, runway three two left, taxi via echo, golf, cross runway one niner right, hold short of runway one niner left. Read it back.",
            situation: "Towered field, you are Concord Ground. The pilot landed 32R but is assigned 32L for departure — muscle memory says 'three two right', and reading back the WRONG parallel is the failure this drill exists to catch. You issued: 'runway three two left, taxi via echo, golf, cross runway one niner right, hold short of runway one niner left'. Grade the readback: assigned runway three two left, the crossing ('cross runway one niner right'), the hold short VERBATIM with its runway ('hold short runway one niner left' — strict at every difficulty), and the callsign. Any missing or wrong runway instruction: ask for that item specifically and do not advance.",
            aircraft: rv12, airport: concord, callType: .taxi
        ),
        Drill(
            ...
            setup: "At Concord, Ground says: RV seven three seven juliet alpha, runway three two left, taxi via golf, hold short runway one niner left. Read it back. Then, when you've taxied down golf and are holding short of one niner left with nothing further said to you, make the call that keeps you moving.",
            situation: "Towered field, you are Concord Ground. Step 1: you issued 'runway three two left, taxi via golf, hold short runway one niner left'. Grade the readback: the hold short VERBATIM with its runway, the assigned runway three two left, and the callsign — strict at every difficulty. Step 2: the pilot is now holding short of one niner left and has NOT been cleared to cross — crossing without explicit clearance is a runway incursion. The right call: 'Concord Ground, RV seven juliet alpha, holding short runway one niner left' (explicitly requesting crossing is fine too). Reply: 'RV seven juliet alpha, cross runway one niner left, continue via papa to runway three two left', then grade the crossing readback ('cross runway one niner left' plus callsign). Set phaseAdvance true only after the crossing clearance has been read back.",
            aircraft: rv12, airport: concord, callType: .taxi
        ),
        Drill(
            id: "t-ccr-taxiback",
            scenario: .towered,
            title: "Taxi back after landing",
            setup: "You've landed on runway three two right at Concord and cleared at echo. You want to taxi back for another departure on three two right. Call ground to request taxi back, then read back the instructions before you move.",
            situation: "Towered field, you are Concord Ground (parallel 32L/32R crossed by 19L/19R; taxiways echo, golf, papa). Step 1: grade the request — Ground, aircraft, position (clear of three two right at echo), and the request (taxi back to runway three two right). Then issue a clearance with teeth, PICKING ONE at random: (a) 'runway three two right, taxi via golf, cross runway one niner right', or (b) 'runway three two right, taxi via golf, hold short of runway one niner right'. Step 2: grade the readback — assigned runway, route, and the cross or hold-short VERBATIM with callsign; if any runway instruction is missing, ask for that item and do not advance. If you chose (b), continue the exchange: after a correct hold-short readback the pilot taxis and holds; when they report holding short (or on your own initiative), issue 'cross runway one niner right, continue via golf' and grade that readback. Set phaseAdvance true only when everything through the crossing has been read back. Coach the habit: write the route down and read it back in the order given.",
            aircraft: rv12, airport: concord, callType: .taxi
        ),
        Drill(
            ...
            setup: "You were cleared to runway three two right at Concord and you're taxiing south on golf when Ground calls: RV seven juliet alpha, runway change — runway three two left, continue via golf, papa, cross runway one niner left. Read back the change.",
            situation: "Towered field, you are Concord Ground. Winds shifted and you amended the clearance mid-taxi: NEW runway three two left, via golf, papa, cross runway one niner left. Grade the readback: the NEW runway ('three two left' — reading back 'three two right' from habit is the exact trap this drill exists for), the crossing verbatim, and the callsign. If they read back the old runway, correct immediately: 'negative, runway three two LEFT, read back'. Set phaseAdvance true only after a full correct readback of the amended clearance.",
            aircraft: rv12, airport: concord, callType: .taxi
        ),
        Drill(
            id: "t-ccr-long-route",
            ...
            setup: "You're at the north ramp at Concord, ready to taxi for departure on runway one niner right. Ground says: RV seven three seven juliet alpha, runway one niner right, taxi via echo, kilo, golf, papa, hold short of runway three two left. Read back the whole thing.",
            situation: "Towered field, you are Concord Ground. You issued a four-segment route: 'runway one niner right, taxi via echo, kilo, golf, papa, hold short of runway three two left'. Grade the readback: the assigned runway, the segments IN ORDER (echo, kilo, golf, papa — a jumbled order gets a specific correction), the hold short VERBATIM with its runway, and the callsign. If segments are dropped, reply 'read back full route'. Set phaseAdvance true only on a complete, ordered readback. Coach the habit: write long routes down as they're read — nobody holds four segments in their head under pressure.",
            aircraft: rv12, airport: concord, callType: .taxi
        ),
        Drill(
